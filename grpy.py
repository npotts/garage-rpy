...
class gsensors:
  ...
  def readValue(self, channel):
    ...
    return (cnt, self.cfg.cnt2val[channel](cnt), self.cfg.spi_units[channel],  self.cfg.spi_names[channel], self.cfg.spi_descriptions[channel])

  def pulseGPIO(self):
    """Pulses the GPIO per the configuration - fork on a thread so we can return quickly"""
    thread.start_new_thread(self.pulseViaThread, ())

  def pulseViaThread(self):
    """Do the actual work"""
  ...
  def dict(self):
    """Read all sensors and return a python dictionary that represents all known samples"""
    rtn = {}
    for i in range(8):
      try:
        (cnt, val, units, name, desc) = self.readValue(i)
      except:
        (cnt, val, units, name, desc) = (None, None, None, None, None)
      rtn[i] = {"counts": cnt, "value": val, "units": units, "name": name, "description": desc}
    return rtn;

  def json(self):
    """Read all the channels configured and returns a json object that contains 
    all metadata and values measured"""
